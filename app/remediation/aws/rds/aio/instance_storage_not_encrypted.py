...
https://aws.amazon.com/blogs/aws/amazon-rds-update-share-encrypted-snapshots-encrypt-existing-instances/
    ...
    async with aioboto3.Session().client('rds',
        ...
        db_instance_snapshot = await rds_client.create_db_snapshot(
        ...
        while db_instance_snapshot['Status'] != 'available':
            ...
            db_instance_snapshot = await rds_client.describe_db_snapshots(
                DBSnapshotIdentifier=snapshot_name)
            db_instance_snapshot = db_instance_snapshot['DBSnapshots'][0]

        db_instance_snapshot_encrypted = await rds_client.copy_db_snapshot(
        ...
        while db_instance_snapshot_encrypted['Status'] != 'available':
            await asyncio.sleep(10)
            db_instance_snapshot_encrypted = await rds_client.describe_db_snapshots(
                DBSnapshotIdentifier=snapshot_name + '-encrypted')
            db_instance_snapshot_encrypted = db_instance_snapshot_encrypted[
                'DBSnapshots'][0]
        response = await rds_client.restore_db_instance_from_db_snapshot(
